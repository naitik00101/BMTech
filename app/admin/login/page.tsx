...
export default function AdminLogin() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate database lookup of whitelist
    setTimeout(() => {
      // In production, you'd check this on the server (Supabase RLS or Edge Function)
      // For this demo, we'll allow registering the first ever device OR checking against localstorage
      const isRegistered = isDeviceAuthorized();
      
      // If no devices registered yet, authorize this one (enrollment phase)
      const allAuthorized = JSON.parse(localStorage.getItem("bmtech_authorized_devices") || "[]");
      
      if (allAuthorized.length === 0) {
        authorizeCurrentDevice();
        window.location.href = "/admin/dashboard";
      } else if (!isRegistered) {
        setError("Unauthorized Device: Access restricted to whitelisted hardware only. Check enrollment keys.");
        setIsLoading(false);
      } else {
        window.location.href = "/admin/dashboard";
      }
        ...
        <div className="bg-surface border border-border rounded-[2.5rem] p-10 md:p-12 shadow-3xl relative overflow-hidden backdrop-blur-xl">
           <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
              ...
              <InputField 
                ...
                type="email" 
                placeholder="admin@bmtech.digital" 
                required 
                className="group"
              />
              <InputField 
